...
async def evaluator_llm(
    ...
    dnum=lines[0].split(':')[1].strip()
    if  dnum.upper().startswith('Draft'.upper()):
        best_draft_num = int(re.search(r'\d+', dnum).group()) - 1   
    else:
        best_draft_num = int(dnum) - 1  # Convert to 0-based index
    rationale = '\n'.join(lines[1:]).strip()
...
def main(query,documents):
    ...
    output, log = asyncio.run(speculative_rag(query, documents))
    return output

if __name__ == "__main__":
    query = """
generate summary documents provided.

"""
    doc1=read_markdown_file(open('text.txt','r') )
    #print(doc1  )
    documents = [  doc1  ]
   
    print(main(query, documents))
